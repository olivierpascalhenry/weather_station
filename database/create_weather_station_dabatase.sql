...
-- creation des tables
CREATE TABLE IF NOT EXISTS public.ext_temp (id integer NOT NULL DEFAULT nextval('ext_temp_id_seq'::regclass), ext_tp_time timestamp without time zone NOT NULL, ext_tp_data real, CONSTRAINT ext_temp_pkey PRIMARY KEY (id)) TABLESPACE pg_default;
ALTER TABLE public.ext_temp OWNER to weather_station;

CREATE TABLE IF NOT EXISTS public.int_hum (id integer NOT NULL DEFAULT nextval('int_hum_id_seq'::regclass), int_hd_time timestamp without time zone NOT NULL, int_hd_data real, CONSTRAINT int_hum_pkey PRIMARY KEY (id)) TABLESPACE pg_default;
ALTER TABLE public.int_hum OWNER to weather_station;

CREATE TABLE IF NOT EXISTS public.int_temp (id integer NOT NULL DEFAULT nextval('int_temp_id_seq'::regclass), int_tp_time timestamp without time zone NOT NULL, int_tp_data real, CONSTRAINT int_temp_pkey PRIMARY KEY (id)) TABLESPACE pg_default;
ALTER TABLE public.int_temp OWNER to weather_station;

CREATE TABLE IF NOT EXISTS public.int_pres (id integer NOT NULL DEFAULT nextval('int_pres_id_seq'::regclass), int_ps_time timestamp without time zone NOT NULL, int_ps_data real, CONSTRAINT int_pres_pkey PRIMARY KEY (id)) TABLESPACE pg_default;
